...
export async function POST(request: Request) {
  ...
  if (file instanceof File) {
    ...
    productImageUrl = `/uploads/${id}.${extension}`;
  }

  const now = new Date().toISOString();
  const steps = createSteps();
  steps[0] = { ...steps[0], status: "completed", progress: 100, startedAt: now, completedAt: now, message: demo ? "Preview product prepared" : "Product image uploaded securely" };
  const run: WorkflowRun = {
    id, name: demo ? "Studio preview" : "Product UGC run", status: "queued", createdAt: now, updatedAt: now,
    prompt, generateAudio, productImagePath, productImageUrl, demo, steps,
  };
  await saveRun(run);
